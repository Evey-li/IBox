...
import com.example.evey.ibox.App;
...
public class ServicePointFragment extends Fragment implements View.OnClickListener, BaiduMap.OnMarkerClickListener, ViewPager.OnPageChangeListener {
    private View mRootView;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mRootView = inflater.inflate(R.layout.fragment_service_point, container, false);
    ...
    private ServicePointBean house;   //房子
    ...
    private int mCurrentLevel = 17;//当前缩放等级
    ...
    public void setListener() {
        ...
        mPoiSearch.setOnGetPoiSearchResultListener(new OnGetPoiSearchResultListener() {
            ...
            public void onGetPoiResult(PoiResult poiResult) {
                ...
                if (allAddr == null) {
                    Toast.makeText(getContext(), "附近没有搜索到您需要搜索的房子", Toast.LENGTH_LONG).show();
                    return;
                }
                if (allAddr.size() == 1) {
                    Toast.makeText(getContext(), "已定位到该商家", Toast.LENGTH_LONG).show();
                } else {
                    Toast.makeText(getContext(), "为你找到" + allAddr.size() + "家房子", Toast.LENGTH_LONG).show();
                }
                ...
                for (PoiInfo p : allAddr) {
                    ...
                    if (isSingleHouse) {
                        ...
                        switch (WAY) {
                            case WAY_BIKE:
                                mapUtils.PaintBikingRoute(getContext(), mBaiduMap, startPoi, endPoi);
                                break;
                            case WAY_CAR:
                                mapUtils.PaintDrivingRoute(getContext(), mBaiduMap, startPoi, endPoi);
                                break;
                            case WAY_WALK:
                                mapUtils.PaintWalkingRoute(getContext(), mBaiduMap, startPoi, endPoi);
                                break;
                            case WAY_BUS:
                                mapUtils.PaintTransitRoute(getContext(), mBaiduMap, startPoi, endPoi);
                                break;
                        }
                        //设置导航按钮可见
    ...
    private void search(String query, int count) {
        if (mLocation == null) {
            if (count == 1) {
                Toast.makeText(getContext(), "正在定位到房子位置..", Toast.LENGTH_LONG).show();
            }
            Toast.makeText(getContext(), "正在尝试为你定位..", Toast.LENGTH_LONG).show();
        }
    ...
    public void onClick(View v) {
        ...
        if (route == null || route.getAllStep() == null) {
            mapUtils.PaintWalkingRoute(getContext(), mBaiduMap, startPoi, endPoi);
            Toast.makeText(getContext(), "自动为您生成步行规划", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private class MyLocationListener implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            //设置当前位置
            location.setLatitude(31.242974);
            location.setLongitude(121.325297);
...
//            startPoi = new LatLng(location.getLatitude(), location.getLongitude());
            startPoi = new LatLng(121.325297, 31.242974);
    ...
    public void addBottomFragments() {
        ...
        for (final ServicePointBean servicePoint : houseList) {
            ...
            titleBottom.setText(servicePoint.getServicePointName());
            summaryBottom.setText(servicePoint.getIntroduce());
            ...
            linearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    DeliveryFragment deliveryFragment = (DeliveryFragment) getActivity().getSupportFragmentManager()
                            .findFragmentByTag("deliveryFragment");
                    Intent intent = new Intent();
                    intent.setAction(Event.CODE_MAP_SERVICE_SELECT);
                    intent.putExtra("data", servicePoint);
